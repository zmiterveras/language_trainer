...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def make_my_view(self, my_view):
        if self.count != 1:
            ...
            my_view.addAction('&' + self.interface_lang['full_view'], self.win.sort_all)
    ...
    def make_my_search(self, my_search):
        if self.count != 1:
            ...
            my_search.addAction(self.interface_lang['translation'], lambda : self.win.on_search(mode='translate'))
    ...
    def lang_choose(self, variant: str, my_menu, my_view, my_search):
        if self.count != 1:
            if not self.check_change():
                return
        if variant == 'en':
            self.win = MyWindowE(desktop, self.app_dir, self.interface_lang, self.sql_handler, variant)
            self.lang = variant
            self.lang_index = 1
            ...
            self.win = MyWindowD(desktop, self.app_dir, self.interface_lang, self.sql_handler, variant)
            self.lang = variant
            self.lang_index = 2
    ...
    def restore_instance_state(self):
        ...
        match self.lang:
            case 'en':
                self.win = MyWindowE(desktop, self.app_dir, self.interface_lang, self.sql_handler, self.lang)
                self.lang_index = 1
            case 'de':
                self.win = MyWindowD(desktop, self.app_dir, self.interface_lang, self.sql_handler, self.lang)
                self.lang_index = 2
